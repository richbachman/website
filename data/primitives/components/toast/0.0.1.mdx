---
metaTitle: Toast
metaDescription: A succinct message that is displayed temporarily and does not require a user response.
name: toast
...
<Description>
  A succinct message that is displayed temporarily and does not require user
  interaction.
</Description>
...
The provider that wraps your toasts and toast region. It usually wraps the application.
...
The fixed area where toasts appear. Users can jump to the region by pressing a hotkey. It is up to you to ensure the discoverability of the hotkey for keyboard users.
...
<PropsTable
  data={[
    ...
    },
    {
      ...
      description: (
        <span>
          ...
          toast region. Use <Code>event.code</Code> value for each key from{' '}
          <Link href="https://keycode.info/" variant="blue" target="_blank">
            keycode.info
          </Link>
          . For meta keys, use <Code>ctrlKey</Code>, <Code>shifKey</Code>, <Code>
            altKey
    ...
    },
    {
      ...
      type: 'string',
      default: '"Toasts"',
      description: 'Text used to label the region for assistive technologies.',
    },
  ]}
/>
    ...
    },
    {
      ...
      description: (
        <span>
          ...
          of the toast for accessibility purposes. For toasts that are triggered
          by the user, choose <Code>foreground</Code>. Toasts generated from
          background tasks should use <Code>background</Code>.
        </span>
      ),
    },
    {
...
An action that does not require user interaction. Notifications that expect a response should use an [`AlertDialog`](/docs/primitives/components/alert-dialog) that is styled to look like a toast instead.

We recommend supplying only one action per toast.
...
<PropsTable
  data={[
    ...
    },
    {
      ...
      type: 'string',
      description:
        'Describe an alternative way to action the button in your application, for screen reader users.',
    },
  ]}
/>
...
Override the default hotkey.

```jsx line=3
<Toast.Provider>
  {/* ... */}
  <Toast.Region __hotkey__={['altKey', 'KeyT']} />
</Toast.Provider>
```
...
Combine `--radix-toast-swipe-move-[x|y]` and `--radix-toast-swipe-end-[x|y]` CSS variables with `data-swipe-[start|move|cancel|end]` attributes to animate a swipe gesture to close. Here's an example using Stitches:

```jsx line=4-7,9-20,24
...
export default () => (
  <ToastPrimitive.Provider __swipeDirection__="right">
    <Toast>...</Toast>
...
For toasts that are triggered by the user, choose `foreground`. Toasts generated from background tasks should use `background`.

#### Foreground

Foreground toasts are announced immediately. Assistive technologies may choose to clear previously queued messages when a foreground toast appears. Try to avoid stacking multiple foreground toasts at the same time.

#### Background

Background toasts are announced at the next graceful opportunity (for example, when the screen reader has finished reading its current sentence) and do not clear queued messages. Overusing them can be perceived as a laggy user experience for screen reader users if used in response to a user interaction.
...
<KeyboardTable
  data={[
    {
      keys: ['F6'],
      description: 'Focuses toasts region.',
    },
    {
      keys: ['Tab'],
...
export const Toast = ({ title, content, children, ...props }) => {
  return (
    <ToastPrimitive.Root {...props}>
      ...
      {children && (
        <ToastPrimitive.Action as={Slot}>{children}</ToastPrimitive.Action>
      )}
...
Create your own imperative API to allow [toast duplication](/docs/primitives/components/toast#consecutive-toasts) if preferred.
  ...
  return (
    <>
      {Array.from({ length: count }).map((_, index) => (
        <ToastPrimitive.Root key={index} {...props}>
